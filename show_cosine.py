...
while True:
    ...
    for encodeFace, faceLoc in zip(encodesCurFrame, facesCurFrame):
        ...
        # Using the face distance directly as the similarity score
        similarity_score = 1 - faceDis[matchIndex]

        if matches[matchIndex] and faceDis[matchIndex] < 0.6:
            name = classNames[matchIndex].upper()
            similarity_text = f' {similarity_score:.4f}'
            print(similarity_score)
        else:
            name = 'UNKNOWN'
            similarity_text = 'Similarity: 0.00'

        print(name, similarity_text)
        y1, x2, y2, x1 = faceLoc
        y1, x2, y2, x1 = y1 * 4, x2 * 4, y2 * 4, x1 * 4
        cv2.rectangle(img, (x1, y1), (x2, y2), (0, 255, 0), 2)
        cv2.rectangle(img, (x1, y2 - 35), (x2, y2), (0, 255, 0), cv2.FILLED)
        cv2.putText(img, name, (x1 + 6, y2 - 45), cv2.FONT_HERSHEY_COMPLEX, 1, (255, 255, 255), 2)
        cv2.putText(img, similarity_text, (x1 + 6, y2 - 10), cv2.FONT_HERSHEY_COMPLEX, 1, (255, 255, 255), 2)
